...
fn try_str_fast_path(
    ...
    if parts.array.len() == parts.named_nodes.len() {
        ...
        let result = TypedValueArrayBuilder::new_with_single_type(
            Arc::clone(&args.encoding),
            TypedValueEncodingField::String.type_id(),
            parts.array.len(),
        )?
        .with_strings(Arc::new(strings))
        .finish();
    ...
    if parts.array.len() == parts.strings.value.len() {
        let strings = StructArray::new(
            TypedValueEncoding::string_fields(),
            vec![
                Arc::new(parts.strings.value.clone()),
                Arc::new(parts.strings.language.clone()),
            ],
            None,
        );
        let result = TypedValueArrayBuilder::new_with_single_type(
...
mod tests {
    ...
    async fn test_str_fast_path_named_node() {
        let encoding = Arc::new(TypedValueEncoding::default());
        let test_vector =
            create_mixed_test_vector(&encoding, Some(TypedValueEncodingField::NamedNode));
        let udf = create_default_builtin_udf(encoding, BuiltinName::Str);
